...
bool ShadowCamera::init(float zoomX, float zoomY, float plane, int depth_text_size)
{
	...
	float sc = sz.width / sz.height;
	_shdwText = RenderTexture3D::create(depth_text_size, depth_text_size * sc, backend::PixelFormat::RGBA8888, backend::PixelFormat::D24S8, RenderTargetFlag::DEPTH);
	//#endif
	_shdwText->retain();
